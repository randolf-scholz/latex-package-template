...
setup target=ROOT_DIR:
    # setup for local usage
    rm -rf "{{target}}/{{TEXMF_DIR}}"
    mkdir -p "{{target}}/{{TEXMF_DIR}}/tex/latex"
    ln -sr "{{ROOT_DIR}}/src/"* "{{target}}/{{TEXMF_DIR}}/tex/latex/"


clean:
    # remove all build and result directories
    find . -type d -name '{{TEST_DIR}}/{{BUILD_DIR}}' -prune -exec rm -rf {} \;
    find . -type d -name '{{TEST_DIR}}/{{RESULT_DIR}}' -prune -exec rm -rf {} \;
